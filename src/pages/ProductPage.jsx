...
import { BACKEND_HOST, BACKEND_PORT, FRONTEND_HOST, FRONTEND_PORT } from "../config";
...
const ProductPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(`http://${BACKEND_HOST}:${BACKEND_PORT}/product/category/${categoryId}`);
  ...
  const handleVariantChange = (productId, variantId) => {
    setSelectedVariant((prev) => ({
      ...
      [productId]: {
        ...
        portion: prev[productId]?.variantId === variantId ? prev[productId].portion : null,
      },
    }));
  };
              ...
              <div className="flex flex-col items-center justify-between gap-10 px-6">
                <div className="flex flex-col space-y-8 text-center">
                  ...
                  <Button
                    ...
                    onClick={() => navigate(`/category/${products[0]?.categoryFK?.menu || ''}`)}
                  >
                    Back to Categories
                  </Button>
                </div>
                    ...
                    >
                      ...
                      <div className="w-full lg:w-3/4 flex flex-col lg:flex-row gap-8">
                        <div className="w-full lg:w-1/2 text-white space-y-4">
                          <h3 className="text-2xl font-semibold text-yellow-300">{product.name}</h3>
                          <p className="text-base text-gray-300">{product.description || "No description"}</p>
                          <p className="text-xl">
                            Price: <span className="text-green-400">${product.price || "N/A"}</span>
                          </p>
                          <p>
                            Promotion: <span className="text-yellow-400">{product.promotion || "None"}</span>
                          </p>
                          <p>
                            Availability: <span className={product.disponibility === "Available" ? "text-green-400" : "text-red-400"}>{product.disponibility || "N/A"}</span>
                          </p>
                          <p>
                        ...
                        {product.recipeFK?.variants?.length > 0 && (
                          <div className="w-full lg:w-1/2 text-white">
                            <h4 className="text-lg font-semibold mb-4 text-yellow-300">Select Variant</h4>
    ...
    >
      <label className="flex items-center gap-4">
        <input
          ...
          checked={selectedVariant[product._id]?.variantId === variant._id}
          onChange={() => {
            if (selectedVariant[product._id]?.variantId === variant._id) {
      ...
      {selectedVariant[product._id]?.variantId === variant._id && variant.portions?.length > 0 && (
          ...
          {variant.portions.map((portion) => (
            ...
            >
              <input
                ...
                checked={selectedVariant[product._id]?.portion === portion}
                onChange={() => handlePortionChange(product._id, portion)}
